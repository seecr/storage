...
class Directory(object):
    def __init__(self, baseDir, baseName, exceptionHandle = DirectoryException):
    ...
    def exists(self):
        return len(self.listFiles()) > 0
    
    def listFiles(self):
        return [ hexStringToString(f.replace(SEPARATOR, '').split('.')[-1]) for f \
            in self._listFiles()]
        
    def removeFile(self, extension):
        filename = self._withFile(extension)
        isfile(filename) and remove(filename)
        self._pruneEmptyDirectories(dirname(filename))
        
    def renameFile(self, srcFile, dstFile):
        srcFilename = self._withFile(srcFile)
        dstFilename = self._withFile(dstFile)
    ...
    def extensionExists(self, extension):
        return isfile(self._withFile(extension))
        
    def openFile(self, extension, mode):
        filename = self._withFile(extension)
